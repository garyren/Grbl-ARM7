/*
  ...
  Based on Grbl, ported to ARM7 by Gary Rensberger

  Copyright (c) 2009-2011 Simen Svale Skogsrud, Gary Rensberger
...
#define VIC_Channel_Timer1  VIC_TIMER1
...
#define ENABLE_STEPPER_DRIVER_INTERRUPT()  T1MCR = TxMCR_INT_ON_MR0 | TxMCR_RESET_ON_MR0;
#define DISABLE_STEPPER_DRIVER_INTERRUPT() T1MCR = TxMCR_RESET_ON_MR0;
...
{   
  ...
  if (bit_istrue(settings.flags,BITFLAG_INVERT_ST_ENABLE)) 
	IOSET = bfENAB;						// enable steppers (active high)   
  else
	IOCLR = bfENAB;						// enable steppers (active low)   
...
void st_go_idle(void) 
{
	...
	if ((settings.stepper_idle_lock_time != 0xff) || bit_istrue(sys.execute,EXEC_ALARM)) 
	{
		...
		if (bit_istrue(settings.flags,BITFLAG_INVERT_ST_ENABLE)) 
			IOCLR = bfENAB;						// disable steppers (active high) 
		else
			IOSET = bfENAB;						// disable steppers 
	}
}
...
// ---------------------------------------------------------------
// "The Stepper Driver Interrupt" - This Timer1 interrupt is the workhorse of Grbl. It is  executed at the rate set with
...
void __attribute__ ((interrupt("IRQ"))) timer1_ISR(void) 
{
	...
	temp = (out_bits & bmDIRMASK);	
	IOCLR = (bmSTEPMASK | bmDIRMASK);
	IOSET = temp;			// set DIR lines first
	IOSET = temp;			// set DIR lines first
	temp = (out_bits & (bmDIRMASK | bmSTEPMASK));

	IOSET = temp;	
	IOSET = temp;	
	IOSET = temp;	
	IOSET = temp;			// delay about 3uS 
	IOSET = temp;	
	IOSET = temp;	
	IOCLR = (bmSTEPMASK);	// then clear step lines to finish the pulse(s)
...
void st_init(void)
{ 
  	IODIR |= STEPPING_MASK | bfRESET | bfENAB | bfMS1 | bfMS2 | bfSPINDLE ;	// they're all outputs
	IOCLR =  STEPPING_MASK | bfRESET | bfSPINDLE;		// step, dir, spindle low initially
	IOSET = bfENAB;
	
	IOCLR = bfMS1;
	IOSET = bfMS2;					// 4 microstep mode
	
	IOCLR = bfRESET;				// start the reset pulse
	IOCLR = bfRESET;				// delay a bit
	IOCLR = bfRESET;				// 
	IOSET = bfRESET;				// release the reset pulse

	T1MR0 = 100-1;     				// Compare-hit at 100uS (-1 reset "tick")
	T1PR  = PCLK/1000000;							// 1us resolution timer
	T1TCR = TxTCR_COUNTER_ENABLE;            		// Timer1 Enable Counting
	VICVectAddr0 = (unsigned long)timer1_ISR;   	// set interrupt vector in slot 0 (highest priority slot)
	VICVectCntl0 = VICVectCntl0_ENABLE | VIC_Channel_Timer1; // use it for Timer 1 Interrupt:
	VICIntEnable = (1<<VIC_Channel_Timer1);    		// Enable Timer1 Interrupt
...
{	
	T1MR0 = cycles;				// setup Timer1 compare match (32 bit, 1us resultution)
	return(cycles);
}
