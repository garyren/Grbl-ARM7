...
{
  LIMIT_DDR &= ~(LIMIT_MASK); // Set as input pins (LPC2106 requires external pullups!)
  #if 0 // ***GMR if (bit_istrue(settings.flags,BITFLAG_HARD_LIMIT_ENABLE)) {
...
#if 0 	// ***GMR  don't have enough pin change interrupts on the LPC2106 available, so this may turn 
		// into a timer task which polls the switches.
...
static void homing_cycle(uint8_t cycle_mask, int8_t pos_dir, bool invert_pin, float homing_rate) 
{
  ...
  out_bits0 ^= (settings.homing_dir_mask & bmDIRMASK); // Apply homing direction settings
  if (!pos_dir) { out_bits0 ^= bmDIRMASK; }   // Invert bits, if negative dir.
  ...
  for(;;) {
    ...
    IOCLR |= (bmSTEPMASK);	// then clear all step lines to finish the pulse(s)
