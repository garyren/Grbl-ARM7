...
void mc_go_home(void)
{
  ...
  //***GMR LIMIT_PCMSK &= ~LIMIT_MASK; // Disable hard limits pin change register for cycle duration
...
//***GMR  if (bit_istrue(settings.flags,BITFLAG_HARD_LIMIT_ENABLE)) { LIMIT_PCMSK |= LIMIT_MASK; }
  // Finished! 
}
